...
% GUI Tool for LMS algorithms ver. 0.7
% Author: Deepankar C
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    % get arbitrary wave
    wave_name = get(handles.arb_wave,'string');
    arbwave = evalin('base',wave_name);
    sigLen = length(arbwave);
    assignin('base','waveform',arbwave);
...
%    if(get(handles.rdob_w,'Value') == 1)
        noise_sig = (-0.5 + rand(1,sigLen)).*sigma;
        plot(noise_sig);
 %   end;

    axis(handles.axes_noise,[0 sigLen -1.5 1.5]);
    title('Noise','FontSize', fontSize);
    xlabel('Time', 'FontSize', fontSize);
    ylabel('Amplitude', 'FontSize', fontSize);
    ...
    plot(ip_sig);
    axis(handles.axes_signal,[0 sigLen -1.5 1.5]);
    title('Input Signal','FontSize', fontSize);
    xlabel('Time', 'FontSize', fontSize);
    ylabel('Amplitude', 'FontSize', fontSize);
...
%   if(get(handles.rdob_w,'Value') == 1)
        ...
        noise_sig = -0.5 + poissrnd(lambda,1,sigLen);
        plot(noise_sig);
%   end;
...
%    sigma = str2double(get(handles.sigma_noise,'string'));
    ...
    % check if selected
    % if(get(handles.rdob_w,'Value') == 1)
        noise_mag = fft(randn(1,sigLen).*sigma).^2;
...
% handles    empty - handles not created until after all CreateFcns called
    ...
    if(lmsAxesStatus == 1)
        ...
        axis(handles.axes_filt_signal,[0 sigLen -1.5 1.5]);
        title('Filtered Signal','FontSize', fontSize);
        xlabel('Time', 'FontSize', fontSize);
        ylabel('Amplitude', 'FontSize', fontSize);
    end;
...
% handles    empty - handles not created until after all CreateFcns called
    ...
    if(lmsAxesStatus == 1)
        ...
        axis(handles.axes_learn_curve,[0 sigLen -1 1.5]);
        title('Learning Curve','FontSize', fontSize);
